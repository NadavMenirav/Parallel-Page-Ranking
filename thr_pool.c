    ...
    for (;;) {
        ...
        while (pool->pool_head == NULL &&
            ...
            if (pool->pool_nthreads <= pool->pool_minimum) {
                ...
                if (pool->pool_linger == 0 || pthread_cond_timedwait(&pool->pool_workcv, &pool->pool_mutex, &ts) == ETIMEDOUT) {
                    timedout = 1;
                    break;
                }
            }
...
thr_pool_t *thr_pool_create(uint32_t min_threads, uint32_t max_threads, uint32_t linger,
                            pthread_attr_t *attr)
{
    thr_pool_t *pool;
...
int thr_pool_queue(thr_pool_t *pool, void *(*func)(void *), void *arg)
{
    ...
    if (pool->pool_idle > 0) {
        ...
        // Create a new worker only if necessary
        if (create_worker(pool) == 0) {
            pool->pool_nthreads++;
        }
    }
...
{
    (void) pthread_mutex_lock(&pool->pool_mutex);
    pthread_cleanup_push((void (*)(void *))pthread_mutex_unlock, (void *)&pool->pool_mutex);
